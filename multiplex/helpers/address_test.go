...
func TestMultiplexServerNewRelayAddress(t *testing.T) {
	...
	addr1, err1 := helpers.NewRelayAddress("tcp://0.0.0.0:1234")
	...
	addr2, err2 := helpers.NewRelayAddress("0.0.0.0:1234")
	...
	addr3, err3 := helpers.NewRelayAddress("wss://0.0.0.0:1234")
	...
	addr4, err4 := helpers.NewRelayAddress("wss://fake-id@0.0.0.0:1234")
...
func TestMultiplexServerNewRelayAddressErrors(t *testing.T) {
	...
	for _, failAddr := range failAddresses1 {
		_, err1 := helpers.NewRelayAddress(failAddr)
		assert.Error(t, err1)
	...
	for _, failAddr := range failAddresses2 {
		_, err2 := helpers.NewRelayAddress(failAddr)
		assert.Error(t, err2)
	...
	for _, failAddr := range failAddresses3 {
		_, err3 := helpers.NewRelayAddress(failAddr)
		assert.Error(t, err3)
...
func TestMultiplexServerRelayAddressString(t *testing.T) {
	// Test 1: fully qualified relay address
	addr1, err1 := helpers.NewRelayAddress("tcp://fake-id@0.0.0.0:1234")
	...
	addr2, err2 := helpers.NewRelayAddress("tcp://0.0.0.0:1234")
