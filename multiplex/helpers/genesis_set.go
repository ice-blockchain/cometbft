...
func ValidateGenesisDocChecksum(database *DBService, genesisDoc *types.GenesisDoc) error {
	...
	genDocSetHashFromDB, err := database.DB().Get(genesisDocHashKey)
	if err != nil {
		return fmt.Errorf(
	...
	if len(genDocSetHashFromDB) == 0 {
		...
		if err = database.DB().SetSync(genesisDocHashKey, genDocChecksum); err != nil {
			return fmt.Errorf(
...
// GenesisDocSetProvider returns a [node.GenesisDocProvider] that loads
...
func GenesisDocSetProvider(nodeCfg *config.Config) func() (*ChecksummedGenesisDocSet, error) {
	return func() (*ChecksummedGenesisDocSet, error) {
