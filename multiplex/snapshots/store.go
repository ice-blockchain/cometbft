...
func NewStore(dir string) (*Store, error) {
	if dir == "" {
		return nil, errors.New("snapshot directory cannot be empty")
	}
...
func (s *Store) Delete(height uint64, format uint32) error {
	...
	if err := os.RemoveAll(s.pathSnapshot(height, format)); err != nil {
		return fmt.Errorf("failed to delete snapshot chunks for height %v format %v: %w", height, format, err)
	}
	if err := os.RemoveAll(s.pathMetadata(height, format)); err != nil {
		return fmt.Errorf("failed to delete snapshot metadata for height %v format %v: %w", height, format, err)
	}
	return nil
}
...
func (s *Store) GetLatest() (*types.Snapshot, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to list snapshot metadata: %w", err)
	}
	if len(metadata) == 0 {
		return nil, nil
	}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to read latest snapshot metadata %s: %w", path, err)
	}

	snapshot := &types.Snapshot{}
	err = proto.Unmarshal(bz, snapshot)
	if err != nil {
		return nil, fmt.Errorf("failed to decode latest snapshot metadata %s: %w", path, err)
	}
	return snapshot, nil
}
...
func (s *Store) List() ([]*types.Snapshot, error) {
	...
	for i, entry := range metadata {
		...
		bz, err := os.ReadFile(path)
		if err != nil {
			return nil, fmt.Errorf("failed to read snapshot metadata %s: %w", entry.Name(), err)
		}
		snapshot := &types.Snapshot{}
		err = proto.Unmarshal(bz, snapshot)
		if err != nil {
			return nil, fmt.Errorf("failed to decode snapshot metadata %s: %w", entry.Name(), err)
		}
...
func (s *Store) Prune(retain uint32) (uint64, error) {
	metadata, err := os.ReadDir(s.pathMetadataDir())
	if err != nil {
		return 0, fmt.Errorf("failed to list snapshot metadata: %w", err)
	}
	...
	for height, ok := range prunedHeights {
		if ok {
			...
			if err != nil {
				return 0, fmt.Errorf("failed to remove snapshot directory for height %v: %w", height, err)
			}
		}
	}
	return pruned, nil
...
func (s *Store) Save(
	...
	if err := os.MkdirAll(s.pathHeight(height), 0o750); err != nil {
		return nil, fmt.Errorf("failed to create snapshot directory for height %v: %w", height, err)
	}
	...
	if err := os.Mkdir(s.pathSnapshot(height, format), 0o750); err != nil {
		return nil, fmt.Errorf("failed to create snapshot directory for height %v format %v: %w", height, format, err)
	}
...
func (s *Store) saveChunk(chunkBody io.ReadCloser, index uint32, snapshot *types.Snapshot, chunkHasher, snapshotHasher hash.Hash) error {
	...
	if err != nil {
		return fmt.Errorf("failed to create snapshot chunk file %q: %w", path, err)
	}
	defer chunkFile.Close()

	chunkHasher.Reset()
	if _, err := io.Copy(io.MultiWriter(chunkFile, chunkHasher, snapshotHasher), chunkBody); err != nil {
		return fmt.Errorf("failed to generate snapshot chunk %d: %w", index, err)
	}

	if err := chunkFile.Close(); err != nil {
		return fmt.Errorf("failed to close snapshot chunk file %d: %w", index, err)
	}

	if err := chunkBody.Close(); err != nil {
		return fmt.Errorf("failed to close snapshot chunk body %d: %w", index, err)
	}
...
func (s *Store) parseMetadataFilename(filename string) (height uint64, format uint32, err error) {
	...
	if err != nil {
		return 0, 0, fmt.Errorf("invalid snapshot metadata filename %s: %w", filename, err)
	}
	var f uint64
