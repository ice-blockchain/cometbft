...
import (
	...
	lightprovider "github.com/ice-blockchain/cometbft/light/provider"
	lighthttp "github.com/ice-blockchain/cometbft/light/provider/http"
	lightrpc "github.com/ice-blockchain/cometbft/light/rpc"
	lightdb "github.com/ice-blockchain/cometbft/light/store/db"
...
func NewLightClientStateProviderWithDBKeyVersion(ctx context.Context,
	...
	for _, server := range servers {
		client, err := rpcClient(ctx, server)
		if err != nil {
...
func (s *lightClientStateProvider) State(ctx context.Context, height uint64) (sm.State, error) {
	...
	primaryRPC, err := rpcClient(ctx, primaryURL)
	if err != nil {
		return sm.State{}, fmt.Errorf("unable to create RPC client: %w", err)
	}
	rpcclient := lightrpc.NewClient(ctx, primaryRPC, s.lc)
...
func rpcClient(ctx context.Context, server string) (*rpchttp.HTTP, error) {
	if !strings.Contains(server, "://") {
		server = "http://" + server
	}
	c, err := rpchttp.New(ctx, server)
	if err != nil {
		return nil, err
	}
	return c, nil
